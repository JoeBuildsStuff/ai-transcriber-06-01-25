-- migration: sync meeting attendees when assigning speakers
-- purpose:
--   1) keep speaker assignment behavior unchanged for meeting_speakers + telemetry.
--   2) automatically ensure assigned speaker contacts exist in meeting_attendees.
-- affected objects:
--   - function ai_transcriber.apply_meeting_speaker_assignment(uuid, integer, uuid, text, text, jsonb, text)
-- special considerations:
--   - only inserts attendees when p_contact_id is not null.
--   - does not remove attendees when a speaker assignment is cleared.
--   - uses a guarded insert to avoid duplicate attendee rows.
