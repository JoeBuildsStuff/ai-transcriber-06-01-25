      ...
      new Plugin<CommentAnchorState>({
        ...
        state: {
          ...
          apply: (tr, pluginState: CommentAnchorState): CommentAnchorState => {
            ...
            ) as CommentAnchorMeta | undefined
            if (meta?.type === 'setThreads') {
              const threads = meta.threads.map((item) =>
                clampAnchor(item, tr.doc.content.size)
              )
              nextState = {
                ...nextState,
                threads,
            ...
            if (tr.docChanged && nextState.threads.length > 0) {
              const mappedThreads = nextState.threads.map(
                (thread: CommentAnchorThread) => {
